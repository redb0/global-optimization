...
class Settings:
    """
    Класс описывающий настройки программы.
    Содержит атрибуты класса для общего доступа и изменения из любого экземпляра.
    Атрибуты:
        - флаг минимизации
        - количество пусков алгоритма
        - размер эпсилон окрестности
        - путь до файла с тестовой функцией
        - координаты глобального минимума
        - координаты глобального максимума
        - положение легенды на графике
        - размерность задачи оптимизации
        - отчет о минимальном максимальном и среденем времени выполнения алгоритма и количества итераций
